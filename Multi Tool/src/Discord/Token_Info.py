...
def ErrorChoice():
    print(f"[{current_time_hour()}] [ERROR] Invalid choice.")
    exit()


def ErrorModule(e):
    print(f"[{current_time_hour()}] [ERROR] Module Import Error: {e}")
    exit()


def Continue():
    input(f"[{current_time_hour()}] Press Enter to continue...")
...
def Error(e):
    print(f"[{current_time_hour()}] [ERROR] {e}")
    exit()


try:
    token_discord = input(f"{BEFORE + current_time_hour() + AFTER} {INPUT} Enter Discord Token -> {reset}")
    ...
    try:
        ...
        print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Error when retrieving information: {white}{e}")
